...
Everyone interacting in the QDYN-pylib project's code base, issue tracker, and any communication channels is expected to follow the [PyPA Code of Conduct](https://www.pypa.io/en/latest/code-of-conduct/).
...
3.  Check <https://github.com/qucontrol/qdynpylib/actions> and make sure that the tests pass for all supported Python versions.

Get Started!
------------

Ready to contribute? Follow [Aaron Meurer's Git Workflow Notes](https://www.asmeurer.com/git-workflow/) (with `qucontrol/qdyn` instead of `sympy/sympy`)

In short,

1.  Clone the repository from `git@github.com:qucontrol/qdyn.git`
...
For developers with direct access to the repository, QDYN-pylib uses a simple branching model where all developments happens directly on the `master` branch. Releases are tags on `master`. All commits on `master` *should* pass all tests and be well-documented. This is so that `git bisect` can be effective. For any non-trivial issue, it is recommended to create a topic branch, instead of working on `master`. There are no restrictions on commits on topic branches, they do not need to contain complete documentation, pass any tests, or even be able to run.
...
QDYN-pylib includes a full test-suite using [pytest](https://docs.pytest.org/en/latest/). We strive for a [test coverage](https://codecov.io/gh/qucontrol/qdyn) above 90%.

From a checkout of the `qdyn` repository you can use
...
~~~ console
tox -e py37-test,py38-test,py39-test
~~~
...
QDYN-pylib could always use more documentation, whether as part of the official docs, in docstrings, or even on the web in blog posts, articles, and such.
...
At any point, from a checkout of the `qdyn` repository, you may run
...
The current version is available through the `__version__` attribute of the `qdyn` package
...
Releases must be tagged in git, using the version string prefixed by "v", e.g. `v1.0.0-dev1` and `v1.0.0`. This makes them available at <https://github.com/qucontrol/qdynpylib/releases>.
