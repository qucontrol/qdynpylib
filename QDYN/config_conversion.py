...
def get_old_config_structure(foldername):
    ...
    ----------
    foldername : string
        Path to folder where to search for \*.fi files
    """
    config_structure = {}
...
def read_old_parser_fi(filename):
    ...
    ----------
    filename : string
        Path to the old parser file that is to be read
    """
    item_list = []
...
def read_old_config(filename):
    ...
    ----------
    filename : string
        Path to the config file that is to be read
    """
    rx_sec = re.compile(r'''
...
def get_mappings(old_config_structure_file='old_config_structure.json',
    ...
    Parameters
    ----------
    old_config_structure_file : string
        Path to the .json file that contains the old config structure stored as
        dictionary
    new_config_structure_file : string
        Path to the .json file that contains the new config structure stored as
        dictionary
    """
    mappings = {}
...
def convert_config(old_config_data, mappings):
    """Convert the old_config_data, as returned by :py:func:`read_old_config`
    into a data structure compatible with the current config file. The
    conversion is performed using a list of mappings, where is encoded to which
    section and item of the new config structure an item from the old config
    structure is assigned

    Parameters
    ----------
    old_config_data :
        Config file data extracted from an old config file by
        :py:func:`read_old_config`
    mappings :
        Dictionary of mappings between old and new section and item names as
        returned by :py:func:`get_mappings`
    """
    new_config_data = {}
